...
3. Holodeck cannot be run with root privileges so after entering the container, before you use holodeck you must login to a new user. The `holodeckuser` with password `holodeck` has been provided. And this account must be used if the `with-worlds` version is selected.
```
