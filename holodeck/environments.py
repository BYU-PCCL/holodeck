...
class HolodeckEnvironment(object):
    ...
    def set_sensor_enabled(self, agent_name, sensor_name, enable):
        ...
        Args:
            ...
            enable (bool): Boolean representing whether to enable or disable the sensor
        """
        if agent_name not in self._sensor_map:
            print("No such agent %s" % agent_name)
        else: 
            enable = sensor_name not in self._sensor_map[agent_name]
            self._should_write_to_command_buffer = True
