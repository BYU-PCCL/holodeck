...
def test_reset():
	...
	for scenario_name in scenario_list:
		...
		sensor_count = sum([len(agent.sensors) for agent in env.agents])

		for _ in range(test_resets):
			env.tick()
			state = env.reset()

			assert almost_equal(init_state, state)
			assert agent_count == len(env.agents)
			assert sensor_count == sum([len(agent.sensors) for agent in env.agents])
